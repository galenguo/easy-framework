...
public class RedisUtils {
    ...
    private static final String DEL_QUEUE_KEY = "delQueueKey";

    private static volatile String delQueueKey = null;

    private static volatile byte[] delByteQueueKey = null;

    private static volatile CommonRedisCommands instance = null;

    public static final String getDelQueueKey() {
        if (delQueueKey == null) {
            synchronized (RedisUtils.class) {
                if (delQueueKey == null) {
                    delQueueKey = Configuration.getProperty("platform") + "_" + DEL_QUEUE_KEY;
                }
            }
        }
        return delQueueKey;
    ...
    public static final byte[] getDelByQueueKey() {
        if (delByteQueueKey == null) {
            synchronized (RedisUtils.class) {
                if (delByteQueueKey == null) {
                    try {
                        delByteQueueKey = getDelQueueKey().getBytes("UTF-8");
     ...
     * @return
     */
    public static Long del(String key, boolean isDelay) {
        Long result = del(key);
        if (result != null && result > 0 && isDelay) {
            //存放key到sortset，一定时间间隔清除一次
            zadd(getDelQueueKey(), System.currentTimeMillis(), key);
        }
        return result;
    }
     ...
     * @return
     */
    public static Long del(byte[] key, boolean isDelay) {
        Long result = del(key);
        if (result != null && result > 0 && isDelay) {
            //存放key到sortset，一定时间间隔清除一次
            zadd(getDelByQueueKey(), System.currentTimeMillis(), key);
        }
        return result;
    }
        ...
        try {
            timer.scheduleAtFixedRate(new TimerTask() {
                ...
                public void run() {
                    ...
                    do {
                        strSet = zrangeByScore(getDelQueueKey(), 0, System.currentTimeMillis() - time, 0, pageSize);
                        if (strSet != null && strSet.size() > 0) {
                            for (String item : strSet) {
                                del(item);
                            }
                        }
                    } while (strSet != null && strSet.size() == pageSize);

                    //删除byte[]类型的key
                    Set<byte[]> byteSet = null;
                    do {
                        byteSet = zrangeByScore(getDelByQueueKey(), 0, System.currentTimeMillis() - time, 0, pageSize);
                        if (byteSet != null && byteSet.size() > 0) {
                            for (byte[] item : byteSet) {
                                del(item);
                            }
                        }
                    } while (byteSet != null && byteSet.size() == pageSize);
                }
            }, 1000 * 60, time);
     ...
     * <pre>
     *     获取不到锁，则一直阻塞等待。
     *     如果permanent为true，锁是永久不失效
     *     如果permanent为false，锁失效时间为1小时。
     * </pre>
     * @param key
     * @param permanent 是否永久锁
     * @return
     */
    public static boolean tryLock(String key, boolean permanent) {
        boolean result = false;
        if (permanent) {
            while (!result) {
                Long temp = getInstance().setnx(key, "1");
                result = (temp != null && temp > 0) ? true : false;
            }
        } else {
            while (!result) {
                String temp = getInstance().set(key, "1", RedisUtils.NX, RedisUtils.EX, TimeUnitSeconds.CACHE_1_HOUR);
     ...
     * 阻塞锁
     * <pre>
     *     获取不到锁，阻塞等待直到获取到锁或者等待超时，锁默认失效时间为1小时。
     * </pre>
     * @param key
     * @param waitingMillis 等待时间（毫秒）
     * @return
     */
    public static boolean tryLock(String key, long waitingMillis) {
        return tryLock(key, waitingMillis, TimeUnitSeconds.CACHE_1_HOUR);
    }

    /**
     * 阻塞锁
     * <pre>
     *     获取不到锁，阻塞等待直到获取到锁或者等待超时，可以设置锁失效时间。
     * </pre>
     *
     * @param key
     * @param waitingMillis  等待时间（毫秒）
     * @param lockExpSeconds 超时时间（秒）
     * @return
     */
    public static boolean tryLock(String key, long waitingMillis, int lockExpSeconds) {
        boolean result = false;
        long startTime = System.currentTimeMillis();
        long currentTime = startTime;
        while ((!result) && (currentTime - startTime < waitingMillis)) {
            String temp = getInstance().set(key, "1", RedisUtils.NX, RedisUtils.EX, lockExpSeconds > 0 ? lockExpSeconds : TimeUnitSeconds.CACHE_1_HOUR);
            result = "OK".equals(temp);
     ...
     * @return
     */
    public static boolean lock(String key, boolean permanent) {
        boolean result = false;
        if (permanent) {
            Long temp = getInstance().setnx(key, "1");
            result = (temp != null && temp > 0) ? true : false;
     ...
     * 非阻塞锁
     * @param key
     * @param lockExpSeconds 锁失效时间（秒）
     * @return
     */
    public static boolean lock(String key, int lockExpSeconds) {
        String result = getInstance().set(key, "1", RedisUtils.NX, RedisUtils.EX, lockExpSeconds > 0 ? lockExpSeconds : TimeUnitSeconds.CACHE_1_HOUR);
    ...
    public static boolean unlock(String key) {
        Long count = getInstance().del(key);
