...
public class IpUtils {
    ...
    private static final String DATABASE_READER_FILEPATH = System.getProperties().getProperty("os.name").toLowerCase()
            .contains("windows") ? "D:/mydownloads" : ApplicationContext.getApplicationPath() + "temp/ip_file";
    ...
    public static void main(String[] args) {
        ...
        System.out.println(" getIpAddress " + "202.167.250.83" + ":" + getIpAddress("202.167.250.83", true));
        // }
    ...
    private static void downloadIpDatabseFile() {
        ...
        while (hasNext && currentCount < 3) {
            ...
            while (bl && count < 3) {
                count++;
                try {
                    FileUtils.downloadNet(DATABASE_READER_FILEPATH, newFileName, FILE_URL);
                    if (newDatabaseFile.exists()) {
                        newDatabaseFile.delete();
                    }
                    GzipUtils.doUncompressFile(DATABASE_READER_FILEPATH + "/" + newFileName);
            ...
            if (!bl) {
                ...
                while (bl && count < 3) {
                    count++;
                    String md5Str = HttpUtils.doGet(FILE_MD5_URL);
                    if (md5Str == null) {
                        bl = true;
                        ...
                        bl = false;
                        if (md5Str.equalsIgnoreCase(MD5Utils.getFileMD5String(newDatabaseFile))) {
    ...
    public static String getIpAddress(String ip, boolean bl) {
        ...
        try {
            ...
            if (cityCN != null) {
                if (bl) {
                    String countryCity= Configuration.getProperty("countryCity");
